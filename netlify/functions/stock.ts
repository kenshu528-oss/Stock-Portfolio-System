...
async function getFinMindStockInfo(symbol: string) {
  try {
    const url = `https://api.finmindtrade.com/api/v4/data?dataset=TaiwanStockInfo&data_id=${symbol}&token=`;
...
async function getFinMindStockPrice(symbol: string) {
  try {
    ...
    const url = `https://api.finmindtrade.com/api/v4/data?dataset=TaiwanStockPrice&data_id=${symbol}&start_date=${startDate.toISOString().split('T')[0]}&token=`;
