...
class StockListUpdateService {
  ...
  private async checkBackendStockList(): Promise<{ isToday: boolean; date?: string }> {
    try {
      ...
      // 🔧 修復：開發環境直接檢查本地檔案
      logger.debug('stock', '開發環境，直接檢查本地股票清單檔案');
      
      try {
        // 檢查 public/stock_list.json 檔案
      ...
      try {
        ...
        logger.debug('stock', '後端 API 不可用', error);
      }
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          logger.debug('stock', '後端更新 API 回應錯誤', { status: response.status });
        }
      } catch (error) {
        logger.debug('stock', '後端更新 API 不可用，嘗試其他方法', error);
      }

      // 🔧 新增：嘗試通過 npm 腳本更新（如果在支援的環境中）
      try {
        // 檢查是否可以執行 npm 腳本
        if (typeof window !== 'undefined' && window.location.hostname === 'localhost') {
          logger.info('stock', '本地開發環境，建議手動執行更新腳本');
          logger.info('stock', '請在終端執行: npm run update:stock-list');
          
          // 在開發環境中，我們不能直接執行 npm 腳本，但可以提供指引
          return false; // 返回 false 讓用戶知道需要手動更新
        }
      } catch (error) {
        logger.debug('stock', '無法執行 npm 腳本', error);
      }
