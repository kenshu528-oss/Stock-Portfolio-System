...
// 批量配置
const BATCH_CONFIG = {
  batchSize: 20,           // 🧪 測試：每批處理 20 支股票（原本 10）
...
export class BatchUpdateService {
  ...
  async batchUpdateStockPrices(
    ...
    // 🔧 去重：同一 symbol 只查一次 API
    const uniqueSymbols = [...new Set(stocks.map(s => s.symbol))];
    const uniqueStocks = uniqueSymbols.map(symbol => stocks.find(s => s.symbol === symbol)!);
    
    logger.info('stock', `開始批量更新: ${stocks.length} 筆記錄，去重後 ${uniqueStocks.length} 支股票`);
    
    // 分批處理（使用去重後的列表）
    const batches = this.splitIntoBatches(uniqueStocks, BATCH_CONFIG.batchSize);
  ...
  private async processBatchSequentially(
    ...
    for (let i = 0; i < batch.length; i++) {
      ...
      try {
        ...
        results.set(stock.symbol, {
          success: false,
          error: (error as Error).message || '獲取失敗'
        });
      }
