...
export const CloudSyncSettings: React.FC<CloudSyncSettingsProps> = ({
  ...
  const handleDownloadFromCloud = async () => {
    ...
    try {
      ...
      console.log('找到雲端資料:', cloudData);
      
      setStatusMessage(`📥 發現雲端資料：帳戶 ${cloudData.accounts?.length || 0} 個，股票 ${cloudData.stocks?.length || 0} 筆`);
      
      console.log('onDataSync 是否存在:', !!onDataSync);
      console.log('雲端資料內容:', cloudData);
      ...
      if (onDataSync) {
        ...
        if (confirmed) {
          console.log('用戶確認同步，調用 onDataSync');
          console.log('onDataSync 函數:', onDataSync);
          console.log('onDataSync 類型:', typeof onDataSync);
          
          if (typeof onDataSync === 'function') {
            console.log('正在調用 onDataSync...');
            try {
              onDataSync(cloudData);
              console.log('onDataSync 調用完成');
              setStatusMessage('✅ 雲端資料已成功同步到本地');
              addOperationLog('success', '✅ 雲端資料已成功同步到本地');
            } catch (syncError) {
              console.error('onDataSync 執行失敗:', syncError);
              setStatusMessage('❌ 資料同步失敗');
              addOperationLog('error', `資料同步失敗: ${syncError instanceof Error ? syncError.message : '未知錯誤'}`);
            }
          } else {
            console.error('onDataSync 不是函數！', onDataSync);
  ...
  const getStatusText = () => {
    if (githubToken) {
      switch (connectionStatus) {
        ...
        default: return '已設定 Token，點擊測試連線';
      }
    } else {
      return '尚未設定雲端同步';
    }
    ...
    >
      <div className="space-y-6">
          ...
          <div className="bg-yellow-900/20 border border-yellow-800 rounded-lg p-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-yellow-300 text-sm mb-3">{cloudSyncAvailability.reason}</p>
                
                {environmentInfo.isGitHubPages && (
                  <div className="space-y-2">
                    <p className="text-yellow-200 text-sm font-medium">💡 如何使用完整功能：</p>
                    <div className="bg-yellow-900/30 rounded-md p-3 text-sm text-yellow-100">
                      ...
                      <p className="text-xs">
                        將應用部署到支援後端的平台（如 Vercel、Netlify）以獲得完整雲端同步功能
                      </p>
                    </div>
                  </div>
                )}
        ...
        <div className="bg-slate-800 rounded-lg p-4">
          <h3 className="text-white font-medium mb-3">環境資訊</h3>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-slate-400">當前環境:</span>
              <span className="text-white">
                {environmentInfo.isLocalDevelopment ? '本機開發' : 
                 environmentInfo.isGitHubPages ? 'GitHub Pages' : '其他'}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">主機名稱:</span>
              <span className="text-white font-mono text-xs">{environmentInfo.hostname}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">雲端同步:</span>
              <span className={cloudSyncAvailability.available ? 'text-green-400' : 'text-red-400'}>
                {cloudSyncAvailability.available ? '✅ 可用' : '❌ 不可用'}
              </span>
            </div>
          </div>
        </div>
        ...
        {cloudSyncAvailability.available ? (
          ...
          /* 不支援環境的替代方案 */
          <div className="bg-slate-800 rounded-lg p-4">
            <h3 className="text-white font-medium mb-3">替代方案</h3>
            <div className="space-y-3 text-sm">
              <div className="flex items-start space-x-3">
                <div className="w-2 h-2 bg-blue-500 rounded-full mt-2 flex-shrink-0"></div>
                <div>
                  <p className="text-slate-200 font-medium">本地匯出/匯入</p>
                  <p className="text-slate-400 text-xs">使用側邊欄的「匯出資料」和「匯入資料」功能進行手動備份</p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                <div className="w-2 h-2 bg-green-500 rounded-full mt-2 flex-shrink-0"></div>
                <div>
                  <p className="text-slate-200 font-medium">本機開發環境</p>
                  <p className="text-slate-400 text-xs">在本機使用 npm run dev 啟動完整功能版本</p>
                </div>
              </div>
              <div className="flex items-start space-x-3">
                <div className="w-2 h-2 bg-purple-500 rounded-full mt-2 flex-shrink-0"></div>
                <div>
                  <p className="text-slate-200 font-medium">完整部署</p>
                  <p className="text-slate-400 text-xs">部署到 Vercel 或 Netlify 等支援後端的平台</p>
                </div>
              </div>
            </div>
          </div>
