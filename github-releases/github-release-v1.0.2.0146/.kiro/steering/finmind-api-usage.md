...
### 識別債券 ETF（v1.0.2.0141 改進）
```javascript
// ✅ 正確：使用正則表達式精確識別債券 ETF
// 格式：00 開頭 + 2-3 位數字 + B 結尾（不區分大小寫）
...
// ✅ 00687B - 債券 ETF  
// ✅ 00937B - 債券 ETF
// ❌ 2330B - 不是債券 ETF（不是 00 開頭）
// ❌ 0050 - 不是債券 ETF（沒有 B 結尾）
```

### Yahoo Finance 後綴策略（v1.0.2.0141 改進）
```javascript
// 債券 ETF：優先使用 .TWO（櫃買中心），備用 .TW
const isBondETF = /^00\d{2,3}B$/i.test(symbol);
const suffixes = isBondETF ? ['.TWO', '.TW'] : ['.TW', '.TWO'];

// 範例：
// 00679B → 嘗試順序：00679B.TWO → 00679B.TW
// 2330   → 嘗試順序：2330.TW → 2330.TWO
```
