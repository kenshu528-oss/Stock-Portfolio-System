...
import { shouldUseBackendProxy, API_ENDPOINTS } from '../config/api';
            ...
            for (let i = 0; i < stocks.length; i++) {
              ...
              try {
                ...
                if (!shouldUseBackendProxy()) {
                  logger.debug('stock', `GitHub Pages 環境，使用外部 API 獲取 ${stock.symbol} 股價...`);
                  
                  // 在 GitHub Pages 環境下，直接使用 StockPriceService
                  const stockPriceService = new (await import('../services/stockPriceService')).StockPriceService();
                  const priceData = await stockPriceService.getStockPrice(stock.symbol);
                  ...
                  if (priceData && priceData.price > 0) {
                    ...
                    state.updateStock(stock.id, {
                      ...
                      lastUpdated: new Date(),
                      priceSource: priceData.source || 'External API'
                    });
                    ...
                    logger.warn('stock', `${stock.symbol} 股價更新失敗: 無法獲取股價`);
                    failCount++;
                  }
                } else {
                  // 使用後端代理（開發環境或 Netlify）
                  const priceController = new AbortController();
                  const priceTimeout = setTimeout(() => priceController.abort(), 10000);
                  
                  const apiUrl = API_ENDPOINTS.getStock(stock.symbol);
                  if (!apiUrl) {
                    logger.warn('stock', `${stock.symbol} 股價更新失敗: API URL 無效`);
                    failCount++;
                    continue;
                  }
