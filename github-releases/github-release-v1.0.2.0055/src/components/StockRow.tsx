...
const StockRow: React.FC<StockRowProps> = ({
  ...
  const handleUpdatePrice = async () => {
    setIsMenuOpen(false);
    try {
      console.log(`開始更新 ${stock.symbol} 股價...`);
      // 調用後端API更新股價
      ...
      if (response.ok) {
        ...
        console.log(`✅ ${stock.symbol} 股價更新成功: ${data.price}`);
      } else {
        console.error(`❌ ${stock.symbol} 股價更新失敗: ${response.status}`);
      }
    } catch (error) {
      console.error(`❌ ${stock.symbol} 更新股價失敗:`, error);
    }
  };
  ...
  const formatGainLoss = (amount: number, percent: number): string => {
    return `${UIEnhancementService.formatNumber(amount, 0)} (${UIEnhancementService.formatPercentage(percent)})`;
  };
