...
const BOND_ETF_PATTERNS = {
  // 債券ETF：00開頭 + 3~4位數字 + B結尾
  BOND_ETF: /^00\d{3,4}B$/i,
  
  // 一般股票型/貨幣型ETF：00開頭 + 3~4位數字（無B結尾）
  // 例如：0050, 0051, 006208, 00940
  REGULAR_ETF: /^0\d{3,5}$/i,
...
export function identifyBondETF(symbol: string, name?: string): BondETFInfo {
  ...
  // 根據代碼模式判斷：債券ETF（00XXX B 結尾）
  if (BOND_ETF_PATTERNS.BOND_ETF.test(upperSymbol)) {
    // 檢查是否為槓桿型
  ...
  // 一般股票型/貨幣型 ETF（0050、006208、00940 等，無 B 結尾）
  // 證交稅率為 0.1%，遠低於一般股票的 0.3%
  if (BOND_ETF_PATTERNS.REGULAR_ETF.test(upperSymbol)) {
    return {
      isBondETF: false,
      isLeveraged: false,
      isInverse: false,
      transactionTaxRate: 0.1, // 股票型 ETF 證交稅率 0.1%
    };
  }
...
export function getBondETFDescription(symbol: string, name?: string): string {
  ...
  if (!bondInfo.isBondETF && bondInfo.transactionTaxRate === 0.1) {
    return '股票型/貨幣型 ETF，證交稅率 0.1%';
  }
...
export const BOND_ETF_CONSTANTS = {
  GENERAL_BOND_TAX_RATE: 0,       // 一般債券ETF證交稅率（免徵至2026/12/31）
  LEVERAGED_BOND_TAX_RATE: 0.1,   // 槓桿型/反向型債券ETF證交稅率
  REGULAR_ETF_TAX_RATE: 0.1,      // 股票型/貨幣型ETF證交稅率
