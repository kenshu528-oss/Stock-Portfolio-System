...
class CloudStockPriceService {
  ...
  /**
   * Vercel Edge Functions 股價獲取 - v1.0.2.0372
   * 使用 Vercel 但在 UI 上顯示為 Yahoo Finance
   */
  private async fetchFromVercel(symbol: string): Promise<StockPrice | null> {
    try {
      const { vercelStockPriceService } = await import('./vercelStockPriceService');
      
      logger.debug('stock', `Vercel Edge Functions 請求: ${symbol}`);
      
      const result = await vercelStockPriceService.getStockPrice(symbol);
      
      if (result && result.price > 0) {
        logger.info('stock', `Vercel Edge Functions 獲取成功: ${symbol}`, { 
          price: result.price,
          displaySource: result.source // 顯示為 "Yahoo Finance"
        });
        
        return result; // source 已經在 vercelStockPriceService 中設為 "Yahoo Finance"
      } else {
        throw new Error('無效的股價資料');
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : '未知錯誤';
      logger.debug('stock', `Vercel Edge Functions 失敗: ${symbol}`, { 
        error: message
      });
      throw new Error(`Vercel Edge Functions 失敗: ${message}`);
    }
  }
