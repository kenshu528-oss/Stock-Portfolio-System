# Stock Portfolio System 規格文檔

## 📋 專案概述

### 基本資訊
- **專案名稱**: Stock Portfolio System
- **版本**: v1.0.2.0244
- **類型**: 現代化股票投資組合管理系統
- **技術棧**: React + TypeScript + Vite + Zustand + TailwindCSS
- **目標用戶**: 個人投資者、股票交易員

### 專案目標
提供一個功能完整、用戶友好的股票投資組合管理系統，支援多帳戶管理、即時股價更新、除權息計算、雲端同步等核心功能。

---

## 🏗️ 系統架構

### 前端架構
```
Stock Portfolio System
├── React 18 (UI框架)
├── TypeScript (類型安全)
├── Vite (建置工具)
├── Zustand (狀態管理)
├── TailwindCSS (樣式框架)
└── Vitest (測試框架)
```

### 後端服務
```
Backend Services
├── Node.js Express Server (股價API代理)
├── GitHub Gist API (雲端同步)
├── FinMind API (台股資料)
├── Yahoo Finance API (國際股價)
└── 證交所 OpenAPI (官方資料)
```

### 資料流架構
```
用戶操作 → React組件 → Zustand Store → 本地存儲/API服務 → 資料更新
```

---

## 🎯 核心功能規格

### 1. 帳戶管理系統

#### 1.1 多帳戶支援
- **功能描述**: 支援建立多個投資帳戶，每個帳戶獨立管理股票
- **預設帳戶**: 系統預設建立「帳戶1」和「帳戶2」
- **帳戶屬性**:
  - 帳戶名稱（可自訂）
  - 證券手續費率（預設 0.1425%）
  - 交易稅率（固定 0.3%）
  - 股票數量統計
  - 建立時間

#### 1.2 帳戶操作
- **新增帳戶**: 支援動態新增帳戶
- **編輯帳戶**: 修改帳戶名稱和費率設定
- **刪除帳戶**: 刪除帳戶及其所有股票記錄
- **帳戶排序**: 支援拖拽排序帳戶順序
- **帳戶切換**: 快速切換當前操作帳戶

### 2. 股票管理系統

#### 2.1 股票記錄結構
```typescript
interface StockRecord {
  id: string;                    // 唯一識別碼
  accountId: string;             // 所屬帳戶ID
  symbol: string;                // 股票代碼
  name: string;                  // 股票名稱
  shares: number;                // 持股數
  costPrice: number;             // 原始成本價
  adjustedCostPrice: number;     // 調整成本價（扣除股息）
  purchaseDate: Date;            // 購買日期
  currentPrice: number;          // 現價
  lastUpdated: Date;             // 最後更新時間
  priceSource: string;           // 價格來源
  dividendRecords?: DividendRecord[]; // 股息記錄
  lastDividendUpdate?: string;   // 最後股息更新時間
  isBondETF?: boolean;           // 是否為債券ETF
}
```

#### 2.2 股票操作功能
- **新增股票**: 支援手動輸入或搜尋新增
- **編輯股票**: 修改持股數、成本價等資訊
- **刪除股票**: 移除股票記錄
- **批次操作**: 支援批次更新股價和除權息資料
- **快速新增**: 提供快速新增常用股票的功能

#### 2.3 股價更新機制
- **即時更新**: 點擊更新按鈕獲取最新股價
- **多重API**: 支援 FinMind、Yahoo Finance、證交所等多個資料源
- **智能後綴**: 根據股票代碼自動判斷市場後綴（.TW/.TWO）
- **錯誤處理**: 優雅處理API失敗和404錯誤
- **更新進度**: 顯示批次更新進度條

### 3. 除權息計算系統

#### 3.1 除權息記錄結構
```typescript
interface DividendRecord {
  id: string;                    // 唯一識別碼
  stockId: string;               // 關聯的股票記錄ID
  symbol: string;                // 股票代碼
  exRightDate: Date;             // 除權息日期
  cashDividendPerShare: number;  // 每股現金股利
  totalCashDividend: number;     // 總現金股利金額
  stockDividendRatio: number;    // 配股比例（每1000股配X股）
  stockDividendShares: number;   // 配得股數
  sharesBeforeRight: number;     // 除權息前持股數
  sharesAfterRight: number;      // 除權息後持股數
  costPriceBeforeRight: number;  // 除權息前成本價
  costPriceAfterRight: number;   // 除權息後調整成本價
  type: 'cash' | 'stock' | 'both'; // 除權息類型
}
```

#### 3.2 計算規則
- **配股計算**: `配股數量 = Math.floor(除權前持股數 × 配股比例 / 1000)`
- **成本價調整**: `調整後成本價 = (除權前成本價 × 除權前股數 - 現金股利總額) / 除權後股數`
- **時間順序**: 除權息記錄必須按時間從舊到新排序計算
- **累積計算**: 每次配股基於上一次的除權後持股數

#### 3.3 除權息模式
- **不含權息模式**: 顯示原始成本價和損益
- **含權息模式**: 顯示調整後成本價和損益（扣除股息影響）

### 4. 投資組合統計

#### 4.1 統計指標
```typescript
interface PortfolioStats {
  totalMarketValue: number;      // 總市值
  totalCost: number;             // 總成本
  totalGainLoss: number;         // 總損益
  totalGainLossPercent: number;  // 總損益率
  totalDividend: number;         // 總股息收入
  totalReturn: number;           // 總報酬（損益+股息）
  todayChange: number;           // 今日變化
  todayChangePercent: number;    // 今日變化率
}
```

#### 4.2 統計功能
- **即時計算**: 根據當前股價即時計算投資組合統計
- **分帳戶統計**: 每個帳戶獨立統計
- **總體統計**: 所有帳戶的總體投資組合統計
- **損益分析**: 支援含權息和不含權息兩種計算模式

### 5. 雲端同步系統

#### 5.1 GitHub Gist 整合
- **Token 管理**: 支援 GitHub Personal Access Token 設定
- **自動搜尋**: 自動搜尋用戶的投資組合 Gists
- **資料上傳**: 將投資組合資料上傳到 GitHub Gist
- **資料下載**: 從 GitHub Gist 下載投資組合資料
- **版本控制**: 利用 Gist 的版本控制功能

#### 5.2 同步功能
- **手動同步**: 用戶主動觸發上傳/下載
- **自動同步**: 支援定時自動同步（可設定間隔）
- **衝突處理**: 提供資料衝突解決機制
- **備份恢復**: 支援從雲端恢復資料

#### 5.3 隱蔽後門功能
- **觸發方式**: 連續點擊使用說明圖示 5 次
- **自動連線**: 自動填入預設 Token 並測試連線
- **自動上傳**: 連線成功後自動上傳資料到雲端
- **自動關閉**: 操作完成後自動關閉對話框

### 6. 資料匯入匯出

#### 6.1 匯出功能
- **JSON 格式**: 匯出完整的投資組合資料
- **Excel 格式**: 匯出為 Excel 試算表
- **備份功能**: 支援完整資料備份

#### 6.2 匯入功能
- **JSON 匯入**: 匯入之前匯出的 JSON 資料
- **資料驗證**: 匯入前驗證資料格式和完整性
- **衝突處理**: 處理匯入資料與現有資料的衝突

### 7. 用戶介面系統

#### 7.1 響應式設計
- **桌面優先**: 主要針對桌面瀏覽器設計
- **移動適配**: 基本的移動設備支援
- **深色主題**: 統一的深色主題設計

#### 7.2 UI 組件規範
- **統一圖示**: 所有圖示使用統一的組件系統（src/components/ui/Icons.tsx）
- **顏色系統**: 
  - 成功/確認：綠色系（bg-green-600）
  - 錯誤/取消：紅色系（bg-red-600）
  - 中性操作：灰色系（bg-slate-600）
- **尺寸標準**: sm(16px), md(20px), lg(24px)
- **過渡效果**: 統一的 hover 和點擊效果

#### 7.3 隱私保護
- **隱私模式**: 預設啟用，隱藏敏感金額資訊
- **快速切換**: 支援一鍵開啟/關閉隱私模式
- **資料保護**: 確保敏感資料不會意外洩露

---

## 🔧 技術規格

### 1. 狀態管理

#### 1.1 Zustand Store 結構
```typescript
interface AppState {
  // UI 狀態
  isSidebarOpen: boolean;
  isAccountManagerOpen: boolean;
  isAddStockFormOpen: boolean;
  
  // 帳戶狀態
  currentAccount: string;
  accounts: Account[];
  
  // 股票狀態
  stocks: StockRecord[];
  
  // 隱私模式
  isPrivacyMode: boolean;
  
  // 成本價顯示模式
  showAdjustedCost: boolean;
  
  // 除權息計算模式
  rightsAdjustmentMode: 'excluding_rights' | 'including_rights';
  
  // 股價更新狀態
  isUpdatingPrices: boolean;
  lastPriceUpdate: Date | null;
  priceUpdateProgress: { current: number; total: number; };
}
```

#### 1.2 持久化策略
- **本地存儲**: 使用 localStorage 持久化狀態
- **版本管理**: 支援狀態版本遷移
- **選擇性持久化**: 只持久化必要的狀態
- **錯誤恢復**: 自動處理損壞的本地資料

### 2. API 整合

#### 2.1 股價 API 優先順序
```
一般股票：
1. Yahoo Finance API（首選）- 即時股價，穩定性佳
2. FinMind API（備用）- 台股專用，中文名稱
3. 證交所 OpenAPI（最後備用）- 官方資料

債券 ETF：
1. Yahoo Finance API（首選）- 配息資料最完整
2. FinMind API（備用）- 部分資料可能不完整
```

#### 2.2 智能股票代碼後綴判斷
```typescript
function getStockSuffixes(symbol: string): string[] {
  const code = parseInt(symbol.substring(0, 4));
  const isBondETF = /^00\d{2,3}B$/i.test(symbol);
  
  if (isBondETF) {
    return ['.TWO', '.TW']; // 債券 ETF 優先櫃買中心
  } else if (code >= 3000 && code <= 8999) {
    return ['.TWO', '.TW']; // 上櫃股票優先櫃買中心
  } else {
    return ['.TW', '.TWO']; // 上市股票優先證交所
  }
}
```

#### 2.3 錯誤處理規範
- **404 錯誤**: 視為正常情況（資料不存在），不輸出警告
- **網路錯誤**: 提供用戶友好的錯誤訊息
- **API 限制**: 實作適當的重試和限流機制

### 3. 除權息計算服務

#### 3.1 統一計算服務
- **單一真相來源**: 所有除權息計算都使用 RightsEventService
- **時間順序**: 除權息記錄必須從舊到新排序
- **累積計算**: 每次配股基於上一次的除權後持股數

#### 3.2 計算流程
```typescript
// 1. 獲取 API 資料並排序
const sortedDividends = apiDividends.sort((a, b) => 
  new Date(a.exDividendDate).getTime() - new Date(b.exDividendDate).getTime()
);

// 2. 初始化變數
let currentShares = stock.shares;
let currentCostPrice = stock.costPrice;

// 3. 按順序計算配股
sortedDividends.forEach(dividend => {
  const { adjustedCostPrice, sharesAfterRight, stockDividendShares } = 
    calculateAdjustedCostPrice(currentCostPrice, currentShares, dividend);
  
  // 累積更新供下一筆使用
  currentShares = sharesAfterRight;
  currentCostPrice = adjustedCostPrice;
});
```

### 4. 日誌系統

#### 4.1 Logger 系統
```typescript
// 日誌等級
LogLevel.ERROR = 0   // 錯誤：必須顯示
LogLevel.WARN = 1    // 警告：重要提示
LogLevel.INFO = 2    // 資訊：一般訊息（預設）
LogLevel.DEBUG = 3   // 調試：詳細資訊
LogLevel.TRACE = 4   // 追蹤：超詳細資訊

// 模組分類
'global', 'dividend', 'stock', 'api', 'cloud', 'import', 'export', 'rights'
```

#### 4.2 使用規範
- **禁止直接使用 console.log**: 統一使用 logger 系統
- **分級控制**: 不同模組可設定不同 log 等級
- **預設簡潔**: 平常只顯示重要訊息（INFO 等級）

---

## 🛡️ 安全與品質規範

### 1. 開發標準

#### 1.1 安全開發原則
- **疊加式開發**: 只添加，不破壞現有功能
- **功能開關**: 新功能可以安全地開啟/關閉
- **快速回滾**: 出問題能立即恢復

#### 1.2 代碼品質檢查
```bash
npm run check:all      # 完整檢查
npm run check:svg      # SVG 格式檢查
npm run check:version  # 版本號一致性檢查
npm run check:state    # 狀態管理檢查
npm run check:rights   # 除權息計算檢查
```

### 2. 版本管理

#### 2.1 版本號規範
- **格式**: v1.0.2.XXXX
- **三處同步**: package.json、version.ts、changelog.ts
- **修改必進版**: 任何功能變更都要更新版本號
- **重新建置**: 版本號更新後必須執行 npm run build

#### 2.2 版本歸檔
- **GitHub 上傳前必須歸檔**: 創建完整的版本資料夾
- **命名規則**: github-release-v1.0.2.XXXX
- **保留歷史**: 所有發布版本都要完整保存

### 3. 備援與恢復

#### 3.1 自動備份策略
- **關鍵操作前自動備份**: 匯入資料前、恢復預設值前、雲端同步前
- **備份存儲位置**: export/backups/
- **備份命名**: auto_backup_YYYYMMDD_HHMMSS.json

#### 3.2 恢復機制
- **自動恢復觸發**: 系統崩潰後重新啟動、檢測到資料異常
- **恢復優先級**: 最新自動備份 → 操作前備份 → 手動備份

---

## 🚀 部署與環境

### 1. 開發環境

#### 1.1 環境要求
- **Node.js**: >= 18.0.0
- **npm**: >= 8.0.0
- **瀏覽器**: Chrome/Firefox/Safari 最新版本

#### 1.2 開發命令
```bash
npm run dev          # 啟動開發服務器
npm run build        # 建置生產版本
npm run preview      # 預覽生產版本
npm run test         # 執行測試
npm run lint         # 代碼檢查
```

### 2. 生產環境

#### 2.1 建置配置
- **建置工具**: Vite
- **輸出目錄**: dist/
- **靜態資源**: 自動優化和壓縮
- **代碼分割**: 自動分割 JavaScript 包

#### 2.2 部署平台
- **主要平台**: Netlify
- **備用平台**: GitHub Pages
- **CDN**: 自動配置內容分發網路

---

## 📊 性能與監控

### 1. 性能指標

#### 1.1 載入性能
- **首次內容繪製 (FCP)**: < 1.5s
- **最大內容繪製 (LCP)**: < 2.5s
- **累積版面偏移 (CLS)**: < 0.1

#### 1.2 運行性能
- **股價更新**: 單次更新 < 3s
- **批次更新**: 10支股票 < 10s
- **資料同步**: 上傳/下載 < 5s

### 2. 錯誤監控

#### 2.1 錯誤類型
- **API 錯誤**: 網路請求失敗
- **資料錯誤**: 格式驗證失敗
- **UI 錯誤**: 組件渲染異常

#### 2.2 錯誤處理
- **優雅降級**: 部分功能失敗不影響整體使用
- **用戶提示**: 提供清楚的錯誤訊息和解決建議
- **自動恢復**: 嘗試自動修復常見問題

---

## 🔮 未來規劃

### 1. 功能擴展

#### 1.1 短期目標（v1.1.x）
- **技術分析**: 添加基本的技術指標
- **報表系統**: 生成投資報表和圖表
- **提醒系統**: 股價提醒和除權息提醒

#### 1.2 中期目標（v1.2.x）
- **多幣別支援**: 支援美股、港股等國際市場
- **API 擴展**: 整合更多資料源
- **行動應用**: 開發 PWA 或原生應用

#### 1.3 長期目標（v2.0.x）
- **AI 分析**: 整合機器學習分析功能
- **社群功能**: 投資組合分享和討論
- **專業版**: 提供進階功能和服務

### 2. 技術升級

#### 2.1 架構優化
- **微前端**: 模組化架構設計
- **服務化**: 後端服務獨立部署
- **快取優化**: 實作更智能的快取策略

#### 2.2 用戶體驗
- **無障礙**: 提升無障礙設計
- **國際化**: 支援多語言介面
- **個人化**: 用戶自訂介面和功能

---

## 📝 附錄

### A. API 文檔
詳細的 API 介面文檔請參考 `docs/API.md`

### B. 開發指南
開發環境設定和貢獻指南請參考 `docs/DEVELOPMENT.md`

### C. 測試規範
測試策略和測試案例請參考 `docs/TESTING.md`

### D. 故障排除
常見問題和解決方案請參考 `docs/TROUBLESHOOTING.md`

---

**文檔版本**: v1.0.2.0221  
**最後更新**: 2026-01-20  
**維護者**: Stock Portfolio System 開發團隊