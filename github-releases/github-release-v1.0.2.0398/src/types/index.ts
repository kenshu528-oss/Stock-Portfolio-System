...
export interface SellTransaction {
  id: string;                    // 唯一識別碼
  accountId: string;             // 所屬帳戶ID
  stockId: string;               // 關聯的買進記錄ID
  symbol: string;                // 股票代碼
  name: string;                  // 股票名稱
  sellDate: Date;                // 賣出日期
  sellShares: number;            // 賣出股數
  sellPrice: number;             // 賣出價格
  sellAmount: number;            // 賣出金額（含手續費、交易稅）
  buyPrice: number;              // 買進成本價（原始）
  buyCost: number;               // 買進成本總額（sellShares × buyPrice）
  brokerageFee: number;          // 手續費
  transactionTax: number;        // 交易稅
  realizedGainLoss: number;      // 已實現損益（不含股息）
  realizedGainLossPercent: number; // 已實現損益率
  dividendIncome: number;        // 持有期間已領股息（按比例分配）
  totalReturn: number;           // 總報酬（損益 + 股息）
...
export interface RealizedPosition {
  ...
  symbol: string;                // 股票代碼
  name: string;                  // 股票名稱
  transactions: SellTransaction[]; // 此股票的所有賣出記錄
  totalBuyCost: number;          // 總買進成本
  totalSellAmount: number;       // 總賣出金額
  totalRealizedGainLoss: number; // 總已實現損益
  totalDividendIncome: number;   // 總已領股息
  totalReturn: number;           // 總報酬
  totalReturnPercent: number;    // 總報酬率
  firstBuyDate: Date;            // 首次買進日期
  lastSellDate: Date;            // 最後賣出日期
}