...
### 11. API 資料完整性實作 (基於 api-standards.md steering 規則)

- [ ] 11.1 實作嚴格的資料驗證機制
  - 禁止使用本地硬編碼股票對照表
  - API 失敗時返回 null 而不是虛假資料
  - 實作友好的錯誤訊息和建議
  - _需求: 7.1, 7.2, 7.3, 7.4_

- [ ]* 11.2 編寫 API 資料完整性的屬性測試
  - **屬性 8: API 資料完整性保證**
  - **驗證: 需求 7.1, 7.2, 7.3**
...
### 12. 日誌記錄標準化 (基於 development-standards.md steering 規則)

- [ ] 12.1 實作統一的日誌記錄系統
  - 使用 logger 系統而不是 console.log
  - 分級控制不同模組的日誌等級
  - 記錄 API 嘗試過程和結果
  - _需求: 1.5, 7.5, 8.2, 8.4_
...
- [ ] 13.3 驗證 steering 規則遵循
  - 確認沒有使用硬編碼股票對照表
  - 確認 API 失敗時不返回虛假資料
  - 確認使用 logger 系統而不是 console.log
  - 確認智能後綴判斷邏輯正確實作
...
### Steering 規則遵循
- [ ] 遵循 api-standards.md：不使用硬編碼對照表
- [ ] 遵循 api-standards.md：API 失敗返回 null
- [ ] 遵循 development-standards.md：使用 logger 系統
- [ ] 遵循 ui-design-standards.md：統一圖示組件
- [ ] 遵循 version-management.md：版本號一致性
