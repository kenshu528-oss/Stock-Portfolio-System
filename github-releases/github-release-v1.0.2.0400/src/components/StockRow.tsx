...
const StockRow: React.FC<StockRowProps> = ({
  ...
  // 注意：證交稅率由 RightsAdjustmentService 內部依股票類型動態判斷
  // 一般股票 0.3%、股票型 ETF 0.1%、債券 ETF 0%（或 0.1% 槓桿型）
  const transactionTaxRate = account?.transactionTax ?? 0.3; // 僅作 fallback，實際由 bondETFService 判斷
  ...
  // 計算損益率（分母為實際買入成本，不含配股）
  // excluding_rights 模式：原始買入股數 × 原始成本價
  // 其他模式：調整後成本價 × 現在持股數
  const totalStockDividendSharesForPercent = RightsAdjustmentService.getTotalStockDividend(stock);
  const totalCostBasisForPercent = (() => {
    if (rightsAdjustmentMode === 'excluding_rights') {
      const originalSharesForPercent = stock.shares - totalStockDividendSharesForPercent;
      return (originalSharesForPercent > 0 ? originalSharesForPercent : stock.shares) * stock.costPrice;
    }
    const costBasisForPercent = stock.adjustedCostPrice || stock.costPrice;
    return costBasisForPercent * stock.shares;
  })();
          ...
          {isDetailRow ? (
            ...
            <div className="flex justify-end items-center gap-1">
              <button
                ...
                className="text-yellow-400 hover:text-yellow-300 transition-colors p-1 rounded hover:bg-slate-600"
                title="賣出此筆記錄"
              >
            ...
            {isMenuOpen && (
              <>
                ...
                {/* 右上角選單 */}
                <div className="fixed top-16 right-4 w-52 bg-slate-800 rounded-lg shadow-2xl z-50 p-3 max-h-[80vh] overflow-y-auto">
                  {/* 更新股價 */}
                  <button
                  ...
                  {!hasMultipleRecords ? (
                    ...
                    <button
                      ...
                      className="w-full mb-3 p-3 text-left text-white hover:bg-yellow-700 transition-colors flex items-center rounded-lg bg-slate-700"
                    >
                      ...
                      <span className="text-sm font-medium">賣出股票</span>
                    </button>
                  ) : (
                    // 合併記錄：提示展開後點擊各筆的 💰
                    <button
                      onClick={() => {
                        setIsMenuOpen(false);
                        if (!isExpanded) onToggleExpansion?.();
                      }}
                      className="w-full mb-3 p-3 text-left text-white hover:bg-yellow-700 transition-colors flex items-center rounded-lg bg-slate-700"
                    >
                      <div className="w-8 h-8 bg-yellow-600 rounded-lg flex items-center justify-center mr-3">
                      ...
                      <div>
                        <div className="text-sm font-medium">賣出股票</div>
                        <div className="text-xs text-yellow-300 mt-0.5">點各筆記錄的 💰 按鈕</div>
                      </div>
                    </button>
                  )}
