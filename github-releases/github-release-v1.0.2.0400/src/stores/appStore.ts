...
const updateStockDividendData = async (stock: StockRecord, state: any, forceRecalculate: boolean = false) => {
  try {
    ...
    logger.warn('dividend', `${stock.symbol} 除權息處理失敗，但不影響股價更新`, msg);
    // 不要拋出錯誤，讓股價更新繼續進行
  }
};
...
export interface AppState {
  ...
  // 賣出交易記錄
  sellTransactions: import('../types').SellTransaction[];
}
...
export interface AppActions {
  ...
  // 賣出操作
  addSellTransaction: (tx: import('../types').SellTransaction) => void;
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        // 賣出操作
        addSellTransaction: (tx) => set((state) => {
          // 如果持股數為 0，從持倉移除該筆記錄
          const stock = state.stocks.find(s => s.id === tx.stockId);
          let newStocks = state.stocks;
          let updatedAccounts = state.accounts;
          
          if (stock) {
            const remainingShares = stock.shares - tx.sellShares;
            if (remainingShares <= 0) {
              // 全部賣完，從持倉移除
              newStocks = state.stocks.filter(s => s.id !== tx.stockId);
              updatedAccounts = state.accounts.map(account => ({
                ...account,
                stockCount: stock.accountId === account.id
                  ? Math.max(0, account.stockCount - 1)
                  : account.stockCount
              }));
            } else {
              ...
              newStocks = state.stocks.map(s =>
                s.id === tx.stockId ? { ...s, shares: remainingShares } : s
              );
            }
          }
          
          return {
            sellTransactions: [...state.sellTransactions, tx],
            stocks: newStocks,
            accounts: updatedAccounts
          };
        }),
        
        deleteSellTransaction: (id) => set((state) => ({
          sellTransactions: state.sellTransactions.filter(tx => tx.id !== id)
        })),
        ...
        updateAllStockPrices: async () => {
          ...
          try {
            ...
            // 🔧 除權息在背景獨立執行，不阻塞 UI 重置
            void (async () => {
              logger.info('dividend', `開始背景處理除權息: ${stocks.length} 支股票`);
              const DIVIDEND_TIMEOUT_MS = 15000; // 每支股票最多等 15 秒
              
              for (let i = 0; i < stocks.length; i++) {
                const stock = stocks[i];
                try {
                  await Promise.race([
                    updateStockDividendData(stock, state, true),
                    new Promise<void>((_, reject) => 
                      setTimeout(() => reject(new Error('除權息處理超時')), DIVIDEND_TIMEOUT_MS)
                    )
                  ]);
                } catch (dividendError) {
                  const msg = dividendError instanceof Error ? dividendError.message : String(dividendError);
                  logger.warn('dividend', `${stock.symbol} 除權息處理失敗: ${msg}`);
                }
                if (i < stocks.length - 1) {
                  await new Promise(resolve => setTimeout(resolve, 100));
                }
              }
              logger.info('dividend', `背景除權息處理完成`);
            })();
      ...
      }),
      {
        ...
        partialize: (state) => ({
          ...
          // ⚠️ 持久化賣出記錄
          sellTransactions: (state.sellTransactions || []).map(tx => ({
            ...tx,
            sellDate: tx.sellDate instanceof Date ? tx.sellDate.toISOString() : tx.sellDate,
          })),
        }),
        // 添加錯誤處理和數據恢復
        onRehydrateStorage: () => (state, error) => {
          ...
          if (state) {
            ...
            // 🔧 恢復賣出記錄的日期
            if (state.sellTransactions) {
              state.sellTransactions = state.sellTransactions.map((tx: any) => ({
                ...tx,
