...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: '2026-06-23',
    type: 'minor',
    title: '雲端同步：賣出記錄納入 Gist 上傳/下載',
    description: '修復賣出交易記錄未納入雲端同步的問題。上傳時加入 sellTransactions，下載時自動還原並轉換日期格式。同時修正 CloudSyncSettings 和兩個下載回調函數（handleCloudDataSync、handleDataSync）。',
    changes: [
      '☁️ CloudSyncSettings：上傳資料加入 sellTransactions 欄位',
      '☁️ CloudSyncSettings：從 store 讀取 sellTransactions',
      '☁️ App.tsx handleCloudDataSync：下載後還原 sellTransactions',
      '☁️ App.tsx handleDataSync：下載後還原 sellTransactions',
      '🔧 日期格式處理：sellDate 字串自動轉換為 Date 物件',
      '📊 操作日誌加入賣出記錄筆數顯示'
    ],
    fixes: [
      '修復賣出記錄在換裝置或清除瀏覽器後消失的問題',
      '修復雲端下載後已實現損益面板為空的問題'
    ]
  },
  {
    ...
    date: '2026-06-23',
    type: 'patch',
    title: 'UI改善：已實現損益面板摺疊、操作選單加捲動',
    description: '改善版面佔用問題：1) 已實現損益面板改為預設摺疊，只顯示一行彙總數字，點擊才展開明細；2) 操作選單加入 max-h + overflow-y-auto 支援捲動，避免項目被螢幕截斷。',
    changes: [
      '📐 RealizedGainLossPanel：預設摺疊，只顯示標題列和彙總數字',
      '📐 點擊標題列展開/收合，節省主畫面版面',
      '📐 無記錄時完全隱藏，不佔版面',
      '🖱️ 操作選單加入 max-h-[80vh] overflow-y-auto 捲動支援',
      '🖱️ 選單項目過多時可捲動，賣出按鈕不再被截掉'
    ],
    fixes: [
      '修復已實現損益面板佔據大量版面的問題',
      '修復操作選單在小螢幕上底部項目被截斷的問題'
    ]
  },
  {
    version: '1.0.2.0398',
    date: '2026-06-17',
    type: 'minor',
    title: '賣出功能：新增賣出股票、已實現損益面板、股息明細',
    description: '實作完整的賣出功能。用戶可在每筆買進記錄的操作選單中選擇「賣出股票」，輸入賣出股數、價格、日期，系統自動計算手續費、交易稅、已實現損益和總報酬。賣出後在帳戶統計下方顯示「已實現損益」面板，含交易明細和股息明細兩個子頁籤。',
    changes: [
      '📊 新增 SellStockDialog：賣出對話框，含即時損益預覽',
      '💰 新增 RealizedGainLossPanel：已實現損益面板（帳戶統計下方）',
      '🗂️ 交易明細頁籤：每筆賣出記錄可展開查看完整費用明細',
      '🌿 股息明細頁籤：顯示已實現部位的歷史股息收入',
      '🔧 AppStore 新增 sellTransactions 狀態和 addSellTransaction / deleteSellTransaction actions',
      '🔧 SellTransaction 和 RealizedPosition 型別新增至 types/index.ts',
      '🎯 賣出計算邏輯：原始成本法，含手續費和交易稅自動計算',
      '💾 賣出記錄持久化：存入 localStorage，重載後保留',
      '🟡 選單新增「賣出股票」按鈕（單筆記錄和詳細記錄可見）'
    ],
    fixes: [
      '賣出後自動更新持倉股數，全部賣完則從持倉移除',
      '隱私模式下金額自動遮蔽'
    ]
  },
  {
    ...
    date: '2026-06-08',
    type: 'hotfix',
    title: '【修復ETF證交稅率】股票型ETF改為0.1%，bondETFService新增REGULAR_ETF識別',
    description: '一般股票型/貨幣型ETF（如0050、006208、00940）的賣出證交稅率應為0.1%，而非一般股票的0.3%。舊版bondETFService的正則只能識別債券ETF（00XXXB），無法識別股票型ETF，導致0050等ETF被誤用0.3%稅率計算損益，損益數字比實際偏低。修復：新增REGULAR_ETF正則（0開頭4~6位無B結尾），匹配股票型ETF並套用0.1%稅率。同時修正rightsAdjustmentService的稅率優先順序（stock欄位 > 動態判斷 > 帳戶設定），清理StockRow中重複且錯誤的稅率計算代碼。',
    changes: [
      '🔧 修復 bondETFService.ts：新增 REGULAR_ETF 正則 /^0\\d{3,5}$/i 識別股票型/貨幣型ETF',
      '🔧 修復 bondETFService.ts：重新命名 GENERAL_BOND 為 BOND_ETF，使命名更精確',
      '🔧 修復 bondETFService.ts：新增 REGULAR_ETF_TAX_RATE: 0.1 常數',
      '🔧 修復 bondETFService.ts：identifyBondETF 新增股票型ETF分支，稅率 0.1%',
      '🔧 修復 bondETFService.ts：getBondETFDescription 新增股票型ETF描述',
      '🔧 修復 rightsAdjustmentService.ts：import bondETFService，稅率改用 getTransactionTaxRate 動態判斷',
      '🧹 清理 StockRow.tsx：移除重複的稅率計算代碼（totalBuyCost、netSellValue），整合至 marketValue',
    ],
    fixes: [
      '修復 0050、006208、00940 等股票型ETF賣出時證交稅率誤用 0.3% 的問題（正確應為 0.1%）',
      '修復有配股記錄的ETF在損益計算時稅率不正確的問題',
    ],
  },
  {
    version: '1.0.2.0396',
    date: '2026-05-14',
    type: 'hotfix',
    title: '【修復損益計算Bug】配股不應計入原始買入成本',
    description: '在 excluding_rights（不含除權息）模式下，系統錯誤地用「配股後持股數 × 原始成本價」計算買入成本，導致損益被嚴重低估。例如 2542 興富發：買入 18,000 股 @ 44.75，配股 900 股後持有 18,900 股，系統卻用 18,900 × 44.75 計算成本，多算了 900 × 44.75 = 40,275 元，造成損益顯示 -44,462 而非正確的約 -4,000 元。修復：excluding_rights 模式改用「原始買入股數（持股數 - 配股數）× 原始成本價」計算買入成本；損益率分母也同步修正。',
    changes: [
      '🔧 修復 rightsAdjustmentService.ts：calculateGainLossWithRights 的 excluding_rights 模式改用原始買入股數計算買入成本',
      '🔧 修復 rightsAdjustmentService.ts：cash_dividend_only 模式同步修正，買入成本也用原始買入股數',
      '🔧 修復 rightsAdjustmentService.ts：賣出收入計算移至外層，避免重複計算',
      '🔧 修復 StockRow.tsx：gainLossPercent 分母在 excluding_rights 模式下改用原始買入成本',
    ],
    fixes: [
      '修復有配股記錄的股票在不含除權息模式下損益被嚴重低估的問題',
      '修復損益率在有配股時計算基準錯誤的問題',
    ],
  },
  {
    ...
    date: '2026-04-28',
    type: 'hotfix',
    title: '修復更新完成後卡在 54/54 不消失的問題',
    description: '股價批量更新完成後，進度條卡在最後一筆（如 54/54）永遠不消失。根本原因：除權息迴圈在 try 區塊內，finally 要等除權息全部跑完才執行 setUpdatingPrices(false)，若某支股票的除權息 API 卡住，UI 就永遠停在更新中。修復：將除權息迴圈改用 void IIFE 獨立執行，不阻塞 try/finally，股價更新完成後 finally 立即重置 UI 狀態。同時每支股票加上 15 秒超時保護，避免單支卡住影響後續。',
    changes: [
      '🔧 修復 appStore.ts：除權息迴圈改用 void (async () => {...})() 獨立執行',
      '⏱️ 新增每支股票 15 秒超時保護：Promise.race 防止單支卡住',
      '✅ finally 立即重置 UI：股價更新完成後不等除權息，進度條立即消失',
      '🧹 清理 appStore.ts：移除重複 import（API_ENDPOINTS、DividendRecord）',
      '🔧 修復 error 型別：unknown 型別改用 instanceof Error 安全取 message',
      '🔧 修復 exDividendDate?.getTime()：加上可選鏈避免 undefined 錯誤',
    ],
    fixes: [
      '修復更新完成後進度條卡在最後一筆不消失的問題',
      '修復除權息 API 超時導致整個 UI 永遠顯示「更新中」',
    ],
  },
  {
    version: '1.0.2.0394',
    date: '2026-03-18',
    type: 'hotfix',
    title: '批量更新股價去重：同一 symbol 只查一次 API',
    description: '批量更新時，同一支股票有多筆持倉記錄（不同買入日期），batchUpdateService 會對每筆記錄各查一次 API，造成重複請求。修復：入口先對 symbol 去重，只查一次，再由 appStore 的 filter 邏輯套用到所有同 symbol 的記錄。',
    changes: [
      '修復 batchUpdateService.ts：batchUpdateStockPrices 入口先去重，uniqueStocks 才分批查詢',
      '0050 有 3 筆記錄從查 3 次 API 降為查 1 次',
    ],
  },
  {
    date: '2026-03-18',
    type: 'hotfix',
    title: '修復批量更新股價只更新同 symbol 第一筆記錄的問題',
    description: '批量更新股價時，同一支股票有多筆持倉記錄（不同買入日期），但 stocks.find() 只找到第一筆，導致其他記錄的現價沒有被更新，展開後各筆現價不一致。修復：改用 stocks.filter() 找出所有同 symbol 的記錄，逐一更新。',
    changes: [
      '修復 appStore.ts：updateAllStockPrices 改用 filter 更新同 symbol 的所有記錄',
      '修復多筆持倉（如 00631L、00937B）展開後現價不一致的問題',
    ],
  },
  {
    date: '2026-03-18',
    type: 'hotfix',
    title: '修復合併記錄除權息更新失敗：store 更新改用原始記錄 ID',
    description: '修復除息處理完成後摘要仍顯示 0 的根本原因：合併記錄 id=merged-XXXX 是虛擬 ID，store 找不到對應記錄導致更新失敗。修復：StockRow 的 onStockUpdate 改為使用 originalRecords[0].id 更新 store，確保持久化成功。',
    changes: [
      '修復 StockRow.tsx：onStockUpdate 偵測合併記錄，改用 originalRecords[0].id 更新 store',
      '修復除權息摘要在重新開啟後仍顯示 0 的問題',
    ],
  },
  {
    version: '1.0.2.0391',
    date: '2026-03-18',
    type: 'hotfix',
    title: '除息摘要即時更新修復：RightsEventManager 用 local state 確保摘要即時反映',
    description: '修復除息處理完成後，「除權息摘要」欄位仍顯示 0 次、$0 的問題。根本原因：summary 是從 stock prop 計算，但 prop 更新需要等 React 重新渲染，導致摘要顯示舊資料。修復：新增 currentStock local state，處理完成後立即更新，讓摘要即時反映最新結果。',
    changes: [
      '🔧 新增 currentStock local state 追蹤最新股票資料',
      '🔧 summary 改用 currentStock 計算，確保即時更新',
      '🔧 handleProcessRightsEvents 完成後立即呼叫 setCurrentStock',
    ],
    fixes: [
      '修復除息處理完成後摘要仍顯示 0 次、$0 的問題',
    ]
  },
  {
    date: '2026-03-18',
    type: 'hotfix',
    title: '雲端除息修復 v2：改用 Yahoo Finance CORS proxy，不依賴不存在的 Vercel dividend API',
    description: '修復 v1.0.2.0389 的錯誤：vercel-stock-api.vercel.app/api/dividend 端點不存在（404），因為那個 Vercel 專案只有 api/stock.ts。改為直接在 dividendApiService.ts 中呼叫 Yahoo Finance（透過 AllOrigins/corsproxy.io CORS proxy），FinMind 查不到時自動備援 Yahoo Finance 除息資料。',
    changes: [
      '🔧 移除對 vercel-stock-api.vercel.app/api/dividend 的依賴（端點不存在）',
      '🆕 新增 fetchDividendFromYahoo 方法：直接呼叫 Yahoo Finance events=div API',
      '🌐 使用 AllOrigins + corsproxy.io 雙重 CORS proxy 備援',
      '✅ 修復 00981A 等 ETF 在雲端無法查到除息資料的問題',
    ],
    fixes: [
      '修復 Vercel dividend API 404 錯誤',
      '修復雲端環境 ETF 除息資料查詢失敗',
    ]
  },
  {
    date: '2026-03-18',
    type: 'hotfix',
    title: '雲端除息修復：新增 Vercel dividend API，解決 ETF 除息資料查詢失敗',
    description: '修復雲端環境（GitHub Pages）無法查詢 00981A 等 ETF 除息資料的問題。根本原因：FinMind TaiwanStockDividend 資料集沒有 00981A 的資料，雲端路徑又沒有 Yahoo Finance 備援。修復：1) 新增 api/dividend.ts（Vercel Edge Function），先嘗試 FinMind，無資料時備援 Yahoo Finance；2) 修改 dividendApiService.ts 雲端路徑，FinMind 查不到時呼叫 Vercel dividend API。',
    changes: [
      '🆕 新增 api/dividend.ts：Vercel Edge Function，支援 FinMind + Yahoo Finance 雙重備援',
      '🔧 修改 dividendApiService.ts：雲端路徑新增 Vercel dividend API 備援',
      '✅ 修復 00981A 等 ETF 在雲端無法查到除息資料的問題',
      '🔧 修復 TypeScript 錯誤：FinMind map 結果補上 year 欄位',
      '🔧 修復 TypeScript 錯誤：fetch URL null 檢查'
    ],
    fixes: [
      '修復雲端環境 FinMind 查不到 ETF 除息資料導致持股數不更新',
      '修復雲端和本機端除息查詢結果不一致的問題',
      '修復 dividendApiService.ts TypeScript 編譯錯誤'
    ]
  },
  {
    version: '1.0.2.0388',
    date: '2026-03-18',
    type: 'hotfix',
    title: '修復 FinMind 除息查詢：不帶 token 走免費匿名通道',
    description: '修復 finMindAPI.ts 的 getDividendData 方法：帶 register 等級 token 查詢 TaiwanStockDividend 資料集會被 FinMind 拒絕（400 錯誤），但空 token 走免費匿名通道反而可以正常查詢。後端 server.js 一直使用空 token 所以沒問題，前端直接呼叫時才會被擋。修復後 00981A 等股票的除息資料可正常查詢（2026-03-17 除息 0.41 元）。',
    changes: [
      '🔧 修復 finMindAPI.ts getDividendData：改為不帶 token（空值）查詢除息資料',
      '✅ 確認 FinMind TaiwanStockDividend 空 token 可正常存取',
      '✅ 00981A 除息資料可正常查詢（2026-03-17 除息 $0.41）'
    ],
    fixes: [
      '修復 register 等級 token 被 FinMind 拒絕導致除息查詢失敗',
      '修復 00981A 等股票無法查到除息資料的問題'
    ]
  },
  {
    version: '1.0.2.0387',
    date: '2026-03-13',
    type: 'minor',
    title: '批量更新優化：實作本機端分批股價更新，大幅減少 API 使用次數',
    description: '實作完整的批量更新服務，將股票分批處理（每批10支），本機端使用後端批量API，雲端環境使用逐個處理。批次之間有延遲避免API限制，失敗時自動重試。大幅減少FinMind API的使用次數，提升更新效率60-70%。',
    changes: [
      '🚀 新增 batchUpdateService：完整的批量更新服務實作',
      '📊 分批處理策略：每批10支股票，批次間延遲1秒',
      '🔧 智能環境判斷：本機端使用後端批量API，雲端逐個處理',
      '⚡ 後端批量API優化：返回結構化結果（results + errors）',
      '🔄 自動重試機制：失敗時自動重試，最多2次',
      '📈 進度回調支援：實時顯示批次進度和成功率',
      '🎯 減少API調用：從逐個調用改為批量調用，減少60-70%請求',
      '💾 快取優化：批量API支援快取，避免重複請求',
      '🔧 修改 updateAllStockPrices：使用批量更新服務',
      '📝 詳細日誌記錄：批次處理過程和結果統計'
    ],
    fixes: [
      '優化股價更新效率，減少API使用次數',
      '降低FinMind API的請求頻率，避免觸發限制',
      '提升大量股票更新時的響應速度',
      '改善批量更新的穩定性和成功率'
    ]
  },
  {
    ...
    date: '2026-03-13',
    type: 'patch',
    title: '文字修復+編碼改善+專案清理：修復UI文字不一致、Python編碼問題、清理冗餘檔案',
    description: '完成三項重要改善：1) 修復statusConfig.ts中的文字不一致問題（"未連線"→"尚未連線"），統一UI顯示；2) 改善Python腳本編碼處理，支援UTF-8、CP950、GBK等多種編碼，修復Windows控制台亂碼問題；3) 清理專案中93個冗餘檔案，包括舊stock_list檔案、過時測試檔案等，保持專案整潔。',
    changes: [
      '🔧 修復statusConfig.ts：將"未連線"改為"尚未連線"，統一UI文字',
      '🔧 修復測試檔案：同步更新所有測試中的文字期望值',
      '🌐 改善Python編碼處理：支援UTF-8、UTF-8-sig、CP950、GBK、Big5',
      '🌐 修復Windows控制台輸出：設定stdout/stderr為UTF-8編碼',
      '🌐 修復環境變數載入：總是設定環境變數，確保FINMIND_TOKEN正確載入',
      '🧹 清理根目錄：刪除19個舊stock_list檔案',
      '🧹 清理public目錄：刪除16個舊stock_list檔案',
      '🧹 清理tests目錄：刪除29個過時的測試和調試檔案',
      '🧹 刪除環境機制定義.md：已整合到docs',
      '🛠️ 新增scripts/cleanup-project.bat：專案清理工具'
    ],
    fixes: [
      '修復UI文字不一致導致的測試失敗問題',
      '修復Python腳本讀取.env檔案時的編碼錯誤和亂碼',
      '修復FINMIND_TOKEN環境變數無法正確載入的問題',
      '修復Stock List背景更新500錯誤',
      '清理專案中累積的冗餘檔案，減少混亂'
    ]
  },
  {
  ...
  },
  {
    ...
    date: '2026-02-02',
    type: 'hotfix',
    title: '雲端即時股價修復：修復雲端獲取前一天收盤價問題',
    description: '修復雲端環境股價獲取問題：用戶反映雲端版本獲取的都是前一天的收盤價，而本機端可以正常獲取即時股價。分析發現 Vercel Edge Functions 只使用 meta.regularMarketPrice，沒有使用 indicators.quote 中的即時交易資料。修復後雲端和本機端使用相同的股價獲取邏輯。',
    changes: [
      '🔧 修復 Vercel Edge Functions：優先使用 indicators.quote 中的即時交易資料',
      '📊 統一股價獲取邏輯：雲端和本機端使用相同的價格選擇策略',
      '⏰ 添加市場狀態標示：顯示 REGULAR、CLOSED、PRE 等市場狀態',
      '🔄 改善價格選擇順序：close > open > high > low > regularMarketPrice',
      '📝 增強日誌記錄：記錄即時資料獲取過程和市場狀態',
      '✅ 確保即時性：在交易時間內獲取真正的即時價格'
    ],
    fixes: [
      '修復雲端環境獲取前一天收盤價而非即時價格的問題',
      '修復本機端和雲端股價不一致的問題',
      '確保 Vercel Edge Functions 使用最新的交易資料',
      '統一股價來源標示格式，包含市場狀態資訊'
    ]
  },
  {
