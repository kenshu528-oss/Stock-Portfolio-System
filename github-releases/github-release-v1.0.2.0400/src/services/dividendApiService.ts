...
export class DividendApiService {
  ...
  /**
   * 從 Yahoo Finance 獲取除息資料（透過 AllOrigins CORS proxy）
   * 適用於雲端環境，FinMind 查不到時的備援
   */
  private static async fetchDividendFromYahoo(symbol: string): Promise<DividendApiRecord[]> {
    // 智能判斷 Yahoo Finance 後綴
    ...
    for (const proxyUrl of proxies) {
      try {
        ...
        if (events && Object.keys(events).length > 0) {
          const dividends: DividendApiRecord[] = Object.values(
            events as Record<string, { amount: number; date: number }>
          ).map((div) => {
            const exDate = new Date(div.date * 1000).toISOString().split('T')[0];
            return {
              symbol,
              exDividendDate: exDate,
              dividendPerShare: Math.round(div.amount * 10000) / 10000,
          ...
          logger.info('dividend', `Yahoo Finance 成功獲取 ${symbol} 除息資料`, { count: dividends.length, proxy: proxyUrl.split('?')[0] });
          return dividends;
        }
      } catch (error) {
        logger.warn('dividend', `Yahoo Finance proxy 失敗: ${proxyUrl.split('?')[0]}`, error);
      }
    }
  ...
  private static async fetchFromAlternativeAPI(symbol: string): Promise<DividendApiRecord[]> {
    try {
      ...
      if (!shouldUseBackendProxy()) {
        ...
        // 🔧 備援：FinMind 查不到時，直接呼叫 Yahoo Finance（透過 AllOrigins CORS proxy）
        // 適用於 FinMind 沒有資料的 ETF（如 00981A）和債券 ETF
        logger.debug('dividend', `${symbol} FinMind 無資料，嘗試 Yahoo Finance 除息資料`);
        const yahooResult = await this.fetchDividendFromYahoo(symbol);
