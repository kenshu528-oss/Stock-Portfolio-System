  ...
  private static async fetchFromAlternativeAPI(symbol: string): Promise<DividendApiRecord[]> {
    try {
      ...
      if (!shouldUseBackendProxy()) {
        logger.debug('dividend', `GitHub Pages 環境，使用 FinMind API 獲取 ${symbol} 股息...`);
        ...
        try {
          ...
          if (dividendData && dividendData.length > 0) {
            ...
            return dividendData.map(item => ({
              ...
              dividendPerShare: item.cashDividendPerShare || 0,
              stockDividendRatio: item.stockDividendRatio || 0,
              year: item.year,
              type: item.cashDividendPerShare > 0 ? 'cash' : 'stock'
            }));
          }
        } catch (error) {
