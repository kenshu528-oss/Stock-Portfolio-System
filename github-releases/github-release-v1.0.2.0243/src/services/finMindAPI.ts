...
export class FinMindAPIProvider implements APIProvider {
   ...
   * @returns 股息資料陣列
   */
  async getDividendData(symbol: string): Promise<any[]> {
    try {
      const url = new URL(FINMIND_CONFIG.baseUrl);
      url.searchParams.set('dataset', 'TaiwanStockDividend');
      url.searchParams.set('data_id', symbol);
      
      logger.debug('dividend', `FinMind 股息資料請求: ${symbol}`);
      ...
      if (!response.ok) {
        if (response.status === 404) {
          logger.debug('dividend', `FinMind ${symbol} 無股息資料 (404)`);
          return [];
        }
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
      ...
      if (data.status !== 200 || !data.data || data.data.length === 0) {
        logger.debug('dividend', `FinMind ${symbol} 無股息資料`);
        return [];
      }
      ...
      }).filter(item => item.exDividendDate); // 過濾掉沒有除息日期的記錄
      
      if (dividends.length > 0) {
        logger.info('dividend', `FinMind 成功獲取 ${symbol} 股息`, { count: dividends.length });
      } else {
        logger.debug('dividend', `FinMind ${symbol} 無有效股息資料`);
      }
