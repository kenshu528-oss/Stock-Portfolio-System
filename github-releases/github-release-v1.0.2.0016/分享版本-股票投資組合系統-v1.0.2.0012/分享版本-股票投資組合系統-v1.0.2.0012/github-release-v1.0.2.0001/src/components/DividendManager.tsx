...
const DividendManager: React.FC<DividendManagerProps> = ({
  ...
  useEffect(() => {
    if (isOpen && DividendDatabaseService.hasDividendData(stock.symbol)) {
      ...
      const existingDates = new Set(
        (stock.dividendRecords || []).map(d => d.exDividendDate.toISOString().split('T')[0])
      );
  ...
  const calculateDividendStats = () => {
    ...
    const yearlyDividend = stock.dividendRecords
      .filter(dividend => dividend.exDividendDate >= oneYearAgo)
          ...
          {stock.dividendRecords && stock.dividendRecords.length > 0 ? (
            <div className="space-y-2">
              {stock.dividendRecords
                .sort((a, b) => b.exDividendDate.getTime() - a.exDividendDate.getTime())
                .map((dividend) => (
                  <div
                    key={dividend.id}
                      ...
                      <div className="flex items-center space-x-4">
                        <div>
                          <p className="text-white font-medium">
                            {dividend.exDividendDate.toLocaleDateString('zh-TW')}
                          </p>
