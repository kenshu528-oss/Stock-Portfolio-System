...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    date: '2026-01-14',
    type: 'minor',
    title: '修正債券 ETF 股息資料來源策略並移除多餘的成本價切換按鈕',
    description: '遵循 STEERING 規則，根據實際測試結果修正債券 ETF 的股息資料來源策略。發現 FinMind 無法取得完整的債券 ETF 配息記錄，而 Yahoo Finance 能提供完整的歷史配息資料。更新策略為：債券 ETF 優先使用 Yahoo Finance API（資料最完整），FinMind 作為備用（可能資料不完整），移除 GoodInfo 爬蟲（反爬蟲嚴格且不穩定）。一般股票維持 FinMind 優先策略不變。同時移除 Header 中多餘的成本價切換按鈕，因為成本價欄位已經同時顯示原始成本和除息後成本，切換按鈕變得多餘且無實際作用。',
    changes: [
      '修正債券 ETF 資料來源順序：Yahoo Finance（首選）→ FinMind（備用）',
      '移除債券 ETF 的 GoodInfo 爬蟲調用：反爬蟲嚴格且不穩定',
      '一般股票維持原策略：FinMind（首選）→ Yahoo Finance（備用）',
      '移除 Header 中多餘的成本價切換按鈕：成本價已同時顯示兩種值',
      '簡化 UI：減少不必要的按鈕，提升用戶體驗',
      '更新 STEERING 規則：finmind-api-usage.md',
      '新增債券 ETF 特殊處理說明：識別規則和資料來源策略',
      '更新後端 server.js：調整債券 ETF 的 API 調用順序',
      '遵循 ui-design-standards 規則：移除冗餘的 UI 元素'
    ],
    fixes: [
      '修復債券 ETF（如 00679B）配息資料不完整的問題',
      '確保債券 ETF 能獲取完整的歷史配息記錄',
      '移除不穩定的 GoodInfo 爬蟲依賴',
      '移除無實際作用的成本價切換按鈕',
      '簡化 Header UI，減少用戶困惑',
      '提升債券 ETF 股息資料的準確性和完整性'
    ]
  },
  {
    version: '1.0.2.0133',
    date: '2026-01-14',
    type: 'critical',
    title: '修復日期類型錯誤並統一強制重新計算功能：Header 批量更新和個股更新使用相同邏輯',
    description: '遵循 STEERING 規則，修復多個日期處理的 TypeScript 錯誤並統一所有更新入口的行為。發現 existingDate 和排序中的 dateA/dateB 可能不是 Date 對象，導致 toISOString 和 getTime 調用失敗。添加安全的日期類型檢查和轉換。同時統一 Header 右上角批量更新按鈕和個股除權息管理按鈕的行為，都使用強制重新計算（forceRecalculate: true），確保所有更新都使用最新的正確計算邏輯，解決舊記錄使用錯誤邏輯導致成本價不正確的問題。簡化 UI，移除重複按鈕。',
    changes: [
      '修復 existingDate.toISOString 錯誤：添加 instanceof Date 檢查',
      '修復 dateA.getTime 錯誤：安全轉換日期對象',
      '修復 mergeDividendRecords 排序邏輯：處理字符串和 Date 對象',
      '統一批量更新邏輯：Header 更新按鈕使用 forceRecalculate: true',
      '統一個股更新邏輯：除權息管理按鈕使用 forceRecalculate: true',
      '簡化 UI：移除重複的更新按鈕，只保留一個主要按鈕',
      '確保功能一致性：所有更新入口都使用相同的強制重新計算邏輯',
      '遵循 safe-development 規則：疊加式功能，不破壞現有邏輯'
    ],
    fixes: [
      '修復 TypeError: existingDate.toISOString is not a function',
      '修復 TypeError: dateA.getTime is not a function',
      '修復 Header 批量更新和個股更新行為不一致的問題',
      '修復舊除權息記錄使用錯誤計算邏輯的問題',
      '確保所有更新都使用最新的正確計算邏輯',
      '解決 00679B 等股票成本價計算錯誤（$31.11 → $29.52）',
      '提升用戶體驗：簡化 UI，移除重複按鈕'
    ]
  },
  {
    ...
    date: '2026-01-14',
    type: 'critical',
    title: '修復除權息調整成本價計算錯誤：避免重複計算導致成本價不正確',
    description: '遵循 STEERING 規則，修復 processRightsAdjustment 函數的嚴重邏輯錯誤。發現在處理多筆除權息時，convertApiRecordToDividendRecord 已經基於當前的 adjustedCostPrice 計算了新的調整後成本價，但 processRightsAdjustment 又使用原始 costPrice 重新計算，導致成本價計算混亂。例如 00679B 有 6 筆股息共 $1.835，正確的調整後成本應該是 $31.39 - $1.835 = $29.555，但顯示為 $31.11（錯誤）。修正為直接使用 dividendRecord.costPriceAfterRight，確保每筆除權息都基於上一筆的除權後成本價累積計算。',
    changes: [
      '修復 processRightsAdjustment 函數：使用 dividendRecord.costPriceAfterRight',
      '移除重複的成本價計算邏輯：避免與 convertApiRecordToDividendRecord 衝突',
      '確保多筆除權息的成本價正確累積計算',
      '修正 00679B 等多筆股息股票的調整後成本價顯示',
      '添加詳細註解說明為何使用 dividendRecord 中的值',
      '遵循 safe-development 規則：修復邏輯錯誤而不破壞現有功能'
    ],
    fixes: [
      '修復調整後成本價計算錯誤：$31.11 → $29.555（00679B 範例）',
      '修復多筆除權息時成本價重複計算的問題',
      '修復 convertApiRecordToDividendRecord 和 processRightsAdjustment 邏輯衝突',
      '確保每筆除權息都基於上一筆的除權後成本價計算',
      '修復用戶反映的成本價顯示不正確問題'
    ]
  },
  {
    ...
    date: '2026-01-14',
    type: 'minor',
    title: '改進 Yahoo Finance 股息 API 支援：新增多種台股符號格式支援，成功支援債券 ETF 股息查詢',
    description: '遵循 STEERING 規則，改進 Yahoo Finance 股息 API 的台股符號格式支援。新增 .TW（上市）、.TWO（上櫃）等多種格式嘗試，成功支援債券 ETF（如 00679B）的股息查詢。整理專案文檔結構，將所有文檔移至 docs/ 資料夾，更新 README 文檔導航，提升專案可維護性。',
    changes: [
      '改進 Yahoo Finance 股息 API：新增多種台股符號格式支援',
      '支援 .TW（上市）、.TWO（上櫃）格式嘗試',
      '成功支援債券 ETF（如 00679B）股息查詢',
      '整理專案文檔結構：移動所有文檔到 docs/ 資料夾',
      '更新 README：添加文檔導航和清楚的資料夾結構說明',
      '提升專案可維護性和文檔可讀性'
    ],
    fixes: [
      '修復債券 ETF 無法從 Yahoo Finance 獲取股息的問題',
      '修復台股符號格式不完整導致的 API 調用失敗',
      '改善專案文檔組織結構'
    ]
  },
  {
