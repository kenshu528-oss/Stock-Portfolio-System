...
const QuickAddStock: React.FC<QuickAddStockProps> = ({
  ...
  const getStockPriceDirectly = async (symbol: string): Promise<number | null> => {
    try {
      ...
      if (isBondETF) {
        ...
        const yahooPrice = await tryYahooFinanceAPI(correctSymbol);
        if (yahooPrice) return yahooPrice;
        
        // 備用：FinMind API
        const finmindPrice = await tryFinMindAPI(symbol);
        if (finmindPrice) return finmindPrice;
      } else {
  ...
  // Yahoo Finance API 調用函數（僅在必要時使用，添加超時控制）
  const tryYahooFinanceAPI = async (correctSymbol: string): Promise<number | null> => {
    try {
        ...
        try {
          ...
          if (proxyResponse.ok) {
            ...
            if (result?.meta?.regularMarketPrice) {
              console.log(`✅ ${correctSymbol} Yahoo Finance 價格: ${result.meta.regularMarketPrice}`);
