...
const RightsEventManager: React.FC<RightsEventManagerProps> = ({
  ...
  // 處理除權息事件
  const handleProcessRightsEvents = async () => {
    console.log('🔄 handleProcessRightsEvents 被調用');
      ...
      <div className="bg-slate-800 rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-4">
            <button
              onClick={() => {
                console.log('🔄 除權息按鈕被點擊');
                console.log('📊 handleProcessRightsEvents 函數:', typeof handleProcessRightsEvents);
                try {
                  handleProcessRightsEvents();
                } catch (error) {
