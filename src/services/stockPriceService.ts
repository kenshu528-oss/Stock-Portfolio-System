...
export class StockPriceService {
  ...
  async getStockName(symbol: string): Promise<string | null> {
    try {
      ...
      const response = await fetch(`${API_CONFIG.BACKEND_PROXY.baseUrl}/search/${symbol}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
        },
      ...
      if (!response.ok) {
        throw new Error(`後端搜尋API錯誤: ${response.status}`);
      }
  ...
  async searchStock(query: string): Promise<StockSearchResult | null> {
    try {
      ...
      if (this.isValidStockSymbol(trimmedQuery)) {
        ...
        const response = await fetch(`${API_CONFIG.BACKEND_PROXY.baseUrl}/search/${trimmedQuery}`, {
          method: 'GET',
          headers: {
            'Accept': 'application/json',
          },
  ...
  private parseYahooResponse(symbol: string, data: any): StockPrice | null {
    try {
      if (data?.chart?.result?.[0]?.meta) {
        ...
        // 嘗試提取股票名稱
        const stockName = meta.longName || meta.shortName || `股票${symbol}`;
